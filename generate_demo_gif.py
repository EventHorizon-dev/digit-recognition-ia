...
try:
    ...
    print("⚠ Le modèle n'existe pas encore. Exécutez 'réseau neuronal from scratch.py' d'abord.")
    exit()
...
draw.text((120, 380), "Cliquez pour dessiner...", fill='black')
frames.append(img.copy())

# ===== FRAME 2-4: Animation de dessin d'un "3" =====
# Créer une image PIL blanche (pour le dessin)
drawing_img = Image.new('L', (300, 300), color=255)
draw_bw = ImageDraw.Draw(drawing_img)

# Simuler un dessin du chiffre "3" avec des points
drawing_points = [
...
for idx in range(0, len(drawing_points), 3):
    ...
    canvas_draw.text((100, 380), "Dessinez un chiffre (0-9)", fill='black')
...
correction_draw.text((80, 200), "Si l'IA se trompe,", fill='black')
correction_draw.text((100, 230), "clique sur le bon chiffre :", fill='black')
...
learning_draw.text((80, 200), "✓ Correction enregistrée!", fill='green')
learning_draw.text((100, 250), "L'IA a appris", fill='black')
learning_draw.text((80, 280), "Nouvelle prédiction: 3 ✓", fill='green')
